...
void init()
{
 int i,j;
    ...
    #pragma omp parallel for collapse(2)  reduction(+:nrColA,nrColB) shared(vresource,vcost,vcostRes,vprice,vbudget)
    ...
    for(i=0;i<n;i++)
	  ...
	  #pragma omp critical
         ...
         #pragma omp parallel for collapse(2) shared(vresource,vcost,vcostRes,vprice,vbudget)
...
T=atoi(argv[1]);    
/*setare nested parallelism true*/
omp_set_nested(1);
/*citire date*/
read_data(argv[2]);
/* prelucrari date*/
