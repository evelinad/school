/*
 * Evelina Dumitrescu SCPD Tema 2 CCG
 *
 * */
...
void opencv(char * image_file)
{
  ...
  gsl_matrix_view GSL_sqA = gsl_matrix_view_array(sqA, N, N);
  gsl_blas_dgemm(CblasNoTrans, CblasNoTrans, 1.0, &GSL_A.matrix, &GSL_A.matrix, 0.0, &GSL_sqA.matrix);
  gsl_blas_dgemm(CblasNoTrans, CblasNoTrans, 3.0, &GSL_sqA.matrix, &GSL_A.matrix, 5.0, &GSL_A.matrix);
