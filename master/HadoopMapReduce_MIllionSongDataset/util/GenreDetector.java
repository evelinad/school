...
public class GenreDetector {
	...
	public ArrayList<Double> computeImportance(String[] terms, String[] freqs) {
                ArrayList<Double> importance = new ArrayList<Double>();
		...
		for (int i = 0; i < terms.length; i++) {
			for (Genre genre: Genre.values()) {
				...
				for (String keyword : genreKeywords[genreID]) {
					if (terms[i].contains(keyword)) {
						importance.set(genreID, importance.get(genreID) + Double.parseDouble(freqs[i]));
					}
				}
			}		
